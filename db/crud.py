...
async def record_match(winner_id: int, loser_id: int, stake: int, outcome: bool) -> dict:
  """Locks both user rows in a fixed id order before updating them, so two concurrent matches touching the same users can't deadlock or lose an update."""
  session_maker = get_session_maker()
  ...
  async with session_maker() as session:
    async with session.begin():
      ...
      new_l_score = max(0, l_score + l_adjustment)
